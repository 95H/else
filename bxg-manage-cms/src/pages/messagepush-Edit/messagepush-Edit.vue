            ...
            <Row class="RadioContent">
              <Col span="19" offset="3">
                ...
                <FormItem label="课程详情：" prop="ContentCourse_id">
                  <div class="InputBox">
                    <FormItem
                      prop="ContentSubject_id"
                      style="display:inline-block;margin-right:1%;width:14%;"
                    >
                      <Select
...
export default {
  ...
  data() {
    return {
      ...
      FormData_tab2: {
        ...
        activityTopic_url: [
          { required: false, message: '请输入活动URL与标题！' }
        ],
        activityTopic_title: [
                ...
                if (res.result == true) {
                  ...
                  this.$Modal.confirm({
                    ...
                    onOk: () => {
                      ...
                      var self = this
                      api.patchMessageModify(this.addData).then(res => {
                        if (res.status == 200) {
                          self.initData()
                          self.$router.push({
                            name: 'message-push',
            ...
            api
              ...
              .then(res => {
                ...
                if (res.result == true) {
                  ...
                  this.$Modal.confirm({
                    title: '您确定要推送该消息么？',
                    content: `<p style="color:red;">将于 ${
                      this.formData3.TimeString
                    } 自动推送</p>`,
                    onOk: () => {
                      this.SubmitNum = 1
                      switch (this.formData2.Release) {
  ...
  created() {
    ...
    api
      ...
      .then(res => {
        let data = res.result
        this.formData.mainTitle = data.title
        this.formData.content = data.content
        this.formData2.Release = data.contentType
        switch (this.formData2.Release) {
          case 'COURSE_DETAIL_PAGE':
            this.formData2.ReleaseContent = '课程详情'
            this.FormData_tab2.ContentCourse_id[0].required = true
            this.formData2.ContentSubject_id = data.menuId
            api
              .getMessageCourses({
                menuId: this.formData2.ContentSubject_id
              })
            ...
            this.formData2.ContentCourse_id = data.courseId
            break
          case 'ACTIVITY_PAGE':
            this.formData2.ReleaseContent = '活动专题'
            this.FormData_tab2.activityTopic_title[0].required = true
            this.FormData_tab2.activityTopic_title[1] = {  
              max: 20, 
              message: '标题不能超过20个字', 
            ...
            this.formData2.activityTopic_title = data.activityTitle
            this.formData2.activityTopic_url = data.activityUrl
            break
          case 'INNER_PAGE':
            this.formData2.ReleaseContent = '应用内部页面'
            this.FormData_tab2.InternalPage_url[0].required = true
            this.formData2.InternalPage_url = data.innerPageType
            break
          case 'LIVE_ROOM_PAGE':
            this.formData2.ReleaseContent = '直播课链接'
            this.FormData_tab2.LiveCourse_id[0].required = true
            this.formData2.LiveSubject_id = data.menuId
            api
              .getMessageLiveList({
                menuId: this.formData2.LiveSubject_id
              })
            ...
            this.formData2.LiveCourse_id = data.liveId

            break
        }
        this.changeReleaseContent()

        this.formData3.ReleaseCrowd = data.group
        this.formData3.ReleaseCrowd_type = data.group
        switch (this.formData3.ReleaseCrowd_type) {
          case 'ALL':
            this.formData3.ReleaseCrowd = '全部人员'
            this.isShowallPersonnel = true
            break
          case 'SUBJECT':
            this.formData3.ReleaseCrowd = '学科'
            this.isShowSubject = true
            var str = data.groupIds
            var arr = str.split(',')
            var arr2 = []
            for (var i = 0; i < arr.length; i++) {
              arr2.push(Number(arr[i]))
            }
            this.formData3.CrowdSubject_id = arr2
            this.changeCrowdSubject()
            break
          case 'COURSE':
            this.formData3.ReleaseCrowd = '课程'
            var str = data.groupIds
            var arr = str.split(',')
            var arr2 = []
            for (var i = 0; i < arr.length; i++) {
            ...
            this.formData3.CrowdCourse_id = arr2
            this.changeCrowdCourse()
            break
          case 'USER':
            this.formData3.ReleaseCrowd = '自定义用户'
            var str = data.groupIds
            var arr = str.split(',')
            var str2 = arr.join('、')
            this.formData3.CustomUser = str2
            break
        }
        this.changeReleaseCrowd()

        if (data.pushTimeType == 1) {
          this.formData3.PushMode = '定时推送'
          this.submit = '定时推送'
          this.isShowTimingPush = true
          this.formData3.TimingPush = new Date(data.pushTime)
        } else if (data.pushTimeType == 0) {
          this.formData3.PushMode = '立即推送'
          this.submit = '立即推送'
        }
      })
  },
