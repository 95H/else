<template>
  <div class="students-wrap">
    <Row :gutter="10" class="baseOption" style="margin-bottom:16px;">
      <Col span="6">
        <Select placeholder="阶段群" v-model="searchParams.groupId" filterable clearable>
          <Option :title="item.name" v-for="(item,index) in groupList" :value="item.id" :key="index">{{ item.name }}</Option>
        </Select>
      </Col>
      <Col span="6">
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      // if (from == 'learning-manage-stat') {
      hasrenwuguanli = this.hasSomeApiAccess('teaching_students_process_data_students_tasks')
      hasxiangqing = this.hasSomeApiAccess('teaching_students_process_data_students_details')
      hasxuejika = this.hasSomeApiAccess('teaching_students_process_data_cards')
      // } 
      ...
      learningTasks(roleType,courseId, userId, stuCourseId, menuId) {
        ...
        if(roleType===1){
          this.$router.push({
            ...
            query: {
              ...obj,
              from: this.fromStr + '_student-list'
            }
          })
        }else{
          this.$router.push({
