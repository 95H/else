<template>
  <div class="stage-stat-container">
    <SimpleTable
      table-height="590"
      :table-columns="columns"
      :table-loading="stageLoading"
      :table-data="stageStats"
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      stageLoading: 'learningStatistics/stageLoading',
      stageError: 'learningStatistics/stageError',
      stageStats: 'learningStatistics/stageStats',
      pageInfo: 'learningStatistics/stagePageInfo'
    }),
  ...
  async created() {
    ...
    const haschakan = this.hasSomeApiAccess('teaching_students_process_data_statistics_phases_latest_details')
    // 表格columns数据
    this.columns = [
      {
        title: '序号',
        ...
        render(h, params) {
          ...
          let len = self.stageStats.length
    ...
    // 获取阶段汇总数据
    let totalStats = await this.fetchStageTotal({
      ...searchParams,
      ...this.fetchParams
    })
  ...
  methods: {
    ...mapMutations({
      resetState: 'learningStatistics/resetStageState'
    }),

    ...mapActions({
      fetchDateList: 'learningStatistics/fetchLatestDateList',
      fetchStageTotal: 'learningStatistics/fetchStageTotal',
      fetchStageStats: 'learningStatistics/fetchStageStats'
    }),
    ...
    async search() {
      ...
      await this.fetchStageStats(params)

      this.stageStats.push(this.totalStats)
    },
...
.stage-stat-container {
  /deep/ tr:last-child td:nth-child(-n + 2) {
    border-right: 0;
  }
