<template>
  <div class="subject-week-stat-container">
    <SimpleTable
      table-height="580"
...
export default {
  ...
  computed: {
    ...mapState({
      tableData: state => state.learningStatistics.subjectWeekStat.stats,
      weekDateList: state => state.learningStatistics.weekDateList,
      pageInfo: state => state.learningStatistics.subjectWeekStat.pageInfo
    }),
  ...
  async created() {
    ...
    // 将weekDateList处理后传给dateRangeOptionList
    _.reduce(
      weekDateList,
  ...
  methods: {
    ...mapMutations({
      resetState: 'learningStatistics/resetSubjectWeekState'
    }),

    ...mapActions({
      fetchWeekSubjectStats: 'learningStatistics/fetchWeekSubjectStats',
    ...
    async fetchTableData() {
      ...
      await this.fetchWeekSubjectStats(params)
