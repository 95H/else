<template>
  <div class="microCourses microCoursesList" ref="scroll">
    ...
    <mt-loadmore class="microCourse"
                 v-if="this.freeMicroCourses.length!=0"
      ...
      <transition-group tag="ul" name="item" class="microCourse-show  cl">
        <microCourseTemp v-for="(item,index) in this.freeMicroCourses" :key="index"
...
<script>
  ...
  fetch({store,route}) {
    return axios({
      ...
      store.commit('getFreeMicroCourses',res.data)
    })
    // return {
    //   getMicroCourses:store.dispatch('getMicroCourses',{isFree:route.query,pageSize:20})
    // }
  },
  mounted(){
    // 站长统计
    ...
    if(this.freeMicroCourses.length<this.paramsData.pageSize){
    	this.loadingText=''
    }
    zhugeTrack('H5-进入课程列表页',{'类型':'免费微课'});
  },
  computed:{
  ...mapState({
      freeMicroCourses:function(state){
        return state.indexModules.index.freeMicroCourses
      }
    })
  },
  methods:{
  ...mapMutations(['getFreeMicroCourses']),
      ...
      if(type=="orderType"){
        this.paramsData.orderType=current==undefined?this.paramsData.orderType:current;
        ...
        this.paramsData.courseLevel=current==undefined?this.paramsData.courseLevel:current;
        ...
        this.paramsData.contentType=current==undefined?this.paramsData.contentType:current;
    ...
    getResult(){
      axios({
        ...
        this.$store.commit('getFreeMicroCourses',res.data);
    ...
    loadBottom(){
      if(this.freeMicroCourses.length!=0){
        this.classTop=true;
        this.paramsData.pageSize+=20;
        this.getResult();
      }
    ...
    filterStyle:function(type,filterId){
      if(type=="isDirec"){
        this.paramsData.directionId=filterId==undefined?this.paramsData.directionId:filterId;
        this.isDirec=filterId;
      }else if(type=="isXueke"){
        this.paramsData.subjectId=filterId==undefined?this.paramsData.subjectId:filterId;
        this.isXueke=filterId;
      }else if(type=="isTag"){
        this.paramsData.tagId=filterId==undefined?this.paramsData.tagId:filterId;
        this.isTag=filterId;
      }
    },
    toggleShow:function(num){
    ...
    shiftData:function(){
      axios.get('/course/getDirections',{params:{courseType:1,isFree:this.paramsData.isFree}}).then((res)=>this.mircoDirections=res.data.resultObject)
      axios.get('/course/getSubjects',{params:{isFree:this.paramsData.isFree}}).then((res)=>this.xueke=res.data.resultObject)
      axios.get('/course/getTags',{params:{isFree:this.paramsData.isFree}}).then((res)=>this.tags=res.data.resultObject)
    }
  }
  }
</script>
