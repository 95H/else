        ...
        login: function (fn) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (MobileLive.isMobile() == 'isMobile') {
                        DWLive.customFns && DWLive.customFns.getUserId(data.datas.viewer)
                        MobileLive.init();
                    } else {
                        LivePlayer.init();
                    }
...
//		直播播放器准备开始播放
    ...
    var MobileLive = {
        ...
        appendVideo: function (s) {
            var video
            // if (DWLive.customFns) {
            //     video = DWLive.customFns.getVideoSrc(s)
            // } else {
            var v = '<video webkit-playsinline playsinline controls x-webkit-airplay="allow" x5-playsinline width="100%" height="100%" src="' + s + '"></video>';
            // var v = '<video webkit-playsinline playsinline controls x-webkit-airplay="allow" x5-playsinline width="100%" height="100%" src="' + s + '" autoplay="true"></video>'
            $('#' + LivePlayer.id).html(v);
            video = document.getElementById('live_player');

            DWLive.customFns.videoAppend(s)
