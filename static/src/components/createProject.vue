			...
			<div>
				<el-form :model="ruleForm" :inline="true" :show-message="false" size="mini" label-position="right" :rules="rules" ref="ruleForm" label-width="105px">
					<el-form-item label="项目名称：" prop="projectName">
						<el-input placeholder="请输入项目名称" v-model="ruleForm.projectName" maxlength="100">
						</el-input>
					</el-form-item>
					...
					<el-form-item label="项目级别：" prop="projectLevelId">
						<el-select v-model="ruleForm.projectLevelId" placeholder="请选择项目级别">
							<el-option v-for="(item,i) in objectList.projectLevelId" :label="item" :value="i" :key="i"></el-option>
						</el-select>
					</el-form-item>
					<el-form-item label="项目性质：" prop="projectNatureId">
						<el-select v-model="ruleForm.projectNatureId" placeholder="请选择项目性质">
							<el-option v-for="(item,i) in objectList.projectNatureId" :label="item" :value="i" :key="i"></el-option>
						</el-select>
					</el-form-item>
					...
					<el-form-item label="评估对象：" prop="showAssessmentType">
						<div class="width240">
							<el-input readonly placeholder="请选择评估对象" @focus="assessmentTargetEvent" v-model="showAssessmentTargetName"
              :style="{width: (assessObj.assessmentTarget.id && assessObj.assessmentTarget.id!== '1320' && (ruleForm.assetType.length || assessObj.assessmentTarget.assetType.length)) ? '85px' : '240px'}"></el-input>
							<el-input readonly v-model="showAssessmentType" v-show="assessObj.assessmentTarget.id && assessObj.assessmentTarget.id!== '1320' && (ruleForm.assetType.length || assessObj.assessmentTarget.assetType.length)"></el-input>
						</div>
					</el-form-item>
					...
					<el-form-item label="评估目的：" prop="assessmentObjectiveId">
						<el-select filterable placeholder="请选择评估目的" v-model="ruleForm.assessmentObjectiveId">
							<el-option v-for="(item,i) in objectList.assessmentObjectiveId" :label="item" :value="i" :key="i"></el-option>
						</el-select>
					</el-form-item>
					<el-form-item label="项目联络人：" prop="contactPersonNames">
						<el-input readonly v-model="contactPersonNames" placeholder="请选择项目联络人" @focus="contactPersonEvent"></el-input>
					</el-form-item>
			...
			<div>
				<el-form :inline="true" size="mini" label-position="right" ref="form" :model="form" label-width="105px">
					...
					<el-form-item label="其他负责人：">
						<el-input readonly v-model="form.projectLeaderName" placeholder="请选择项目其他负责人" @focus="selectPersons('projectLeader')"></el-input>
					</el-form-item>
					...
					<el-form-item label="经济性质：">
						<el-select placeholder="请选择经济性质" v-model="form.billEconomicNatureId">
							<el-option v-for="(item,i) in objectList.billEconomicNatureId" :label="item" :value="i" :key="i"></el-option>
						</el-select>
					</el-form-item>
					...
					<el-form-item label="经济性质改变：">
						<el-radio-group v-model="form.isNatureChange">
							<el-radio label="1">改变</el-radio>
							<el-radio label="0">不改变</el-radio>
						</el-radio-group>
					</el-form-item>
					...
					<el-form-item label="项目背景：" class="project-background-hook" :style="styleAuto">
						<el-input type="textarea" maxlength="500" v-model="form.projectBackgroud" placeholder="请输入项目背景" :style="styleObj"></el-input>
					</el-form-item>
					...
					<div class="add reset-btn" @click="addSubProject()">+ 添加</div>
					<div class="title">委托方信息：</div>
					<div class="content add-client" v-for="(item , i) in othercompanyJson" :key="'a'+i">
						<li>
							<label>{{nameArr[0]}}</label>
							<el-input v-model="item.companyName" placeholder="请输入委托方名称"></el-input>
						</li>
						<li>
							<label>{{nameArr[1]}}</label>
							<el-select placeholder="请选择经济性质" v-model="item.economicNatureId">
								<el-option v-for="(item,index) in objectList.billEconomicNatureId" :label="item" :value="index" :key="index"></el-option>
							</el-select>
						</li>
						<li>
					...
					<div class="add reset-btn" @click="addClient()">+ 添加</div>
					<div class="title">支付方信息：</div>
					<div class="content add-client" v-for="(item , i) in payerJson" :key="'b'+i">
						<li>
							<label>{{nameArr[0]}}</label>
							<el-input v-model="item.billName" placeholder="请输入支付方名称"></el-input>
						</li>
						<li>
							<label>{{nameArr[1]}}</label>
					...
					<div class="add reset-btn" @click="addPayment()">+ 添加</div>
					<div class="title">合同附件信息：</div>
					<div class="add-client fileCont">
            <div class="contract-wrapper">
              <span class="title-item">合同附件信息：</span>
            </div>
						<el-upload class="upload-demo" action="/api/file/upload" :on-remove="delFile" :on-success="successFile" :on-error="errorFile"
                       :data="fileData" :headers=fileObj :file-list="fileList" :class="fileList.length ? '' : 'no-border'" :before-upload="beforeAvatarUpload">
					...
					<div class="content add-client projectmsg">
						...
						<el-form-item label-width="129px" class="member-competent-hook" label="项目组人员专业胜任能力及独立性：" :style="styleLine">
		...
		<center-dialog ref="contactPersonDialog" dialogWidth='500px' dialogHeight="400px" dialogTitle="请选择项目联系人" titlePosition="left">
          ...
          required: true, message: '域名不能为空', trigger: 'blur'
						...
						<div class="icon-container">
							<i class="iconfont icon-shanchu1" @click.prevent="removeDomain(index)"></i>
						</div>
					</el-form-item>
					<el-form-item>
						<div class="add reset-btn" @click="addDomain" v-if="contactPersonObj.contactPerson.length < 5">+添加</div>
					</el-form-item>
					<div v-if="contactPersonObj.contactPerson.length" class="button-container">
						<a class="search-btn" @click="handlePersonEvent(true)">确 定</a>
						<a class="reset-btn" @click="handlePersonEvent(false)">取 消</a>
					</div>
				</el-form>
			</div>
		</center-dialog>
...
<script>
	...
	export default {
		...
		data() {
			const _this = this;
			//评估对象正则验证
			const assessmentTargetName = (rule, value, callback) => {
				switch(_this.assessObj.assessmentTarget.id) {
					//1320:"整体资产"
					//          1321:"部分资产"
					//            1322:"单项资产"
					case "1321":
						if(_this.assessObj.assessmentTarget.assetType.length >= 2) {
							callback();
						} else {
							callback(new Error());
						}
			...
			return {
				...
				ruleForm: {
					...
					assessmentBaseDate: '',
					projectLevelId: '',
					showAssessmentTargetName: '', //必填项字段
					assessmentTarget:'',
					assetType: "", //必填项字段
					projectNatureId: '',
					issueAssetsReport: '',
...
// 					travelExpenseExpect: '',
          ...
          showAssessmentType: [{
						validator: assessmentTargetName,
						trigger: 'change'
					}],
					projectLevelId: [{
						required: true,
...
// 					}],
		...
		computed: {
			//计算预期合同价
// 			contract() {
// 			  const contract =  (this.ruleForm.evaluationFeeExpect * 100 + this.ruleForm.travelExpenseExpect * 100) / 100;
// 				return this.ruleForm.projectExpenseExpect = contract ? contract : ""
// 			},
		...
		methods: {
			...
			childMethod() {
				...
				this.$refs['ruleForm'].validate((valid) => {
					if(valid) {
						this.$http.post('/api/project/verificationProjectName', {
							...
							.then(function(res) {
								if(res.data > 0) {
									_this.$message({
										showClose: true,
										type: 'warning',
										message: '项目名称已存在，请重新编辑'
									});
								} else if(res.data == 0) {
									_this.$confirm('确认创建此项目？', '提示', {
										confirmButtonText: '确定创建',
										cancelButtonText: '取消',
										type: 'success'
									}).then(() => {
										var setupprojectJson = JSON.stringify(_this.ruleForm);
										var otherProjectJson = JSON.stringify(_this.form);
										var othercompanyJson = JSON.stringify(_this.othercompanyJson);
										var payerJson = JSON.stringify(_this.payerJson);
										var contractDocJson = JSON.stringify(_this.fileMsg)
										var cache = [];
										var subProjectJson = JSON.stringify(_this.dataSource, function(key, value) {
										...
										_this.$http.post('/api/projectRequest/setupSaveSetupRequest', {
...
//										});
			...
			setPersonId(type, name, id) {
				...
				this.contactPersonObj.contactPerson[this.contactPersonObj.contactPersonIndex]
			},
...
// 					})
			...
			successFile(response, file, fileList) {
				...
				for(var i in fileList) {
					...
					obj.filePath = fileList[i].response.data.fileDir;
					arr.push(obj);
				}
				this.fileMsg = arr;
			},
...
<style scoped="scoped" lang="scss">
	.center-body {
      ...
      /deep/ .el-table__body-wrapper{
        .el-table__row:hover>td{background-color: #FFF5EB !important;}
        .input:focus{
          outline: none;
