		...
		methods:{
			...
			selectedPreson(data){
				if(!data.isDept){
					...
					this.selectPersonName = data.name;
					this.selectPersonId = data.id;
				}else{
			...
			//确定
			surePreson(){
				// this.$emit('setGroupId', this.type,this.selectPersonName,this.selectPersonId);
				this.$emit('setGroupId', this.selectPersonName,this.selectPersonId);
			...
			showPersonDialog(chargeAppraiser,id,name){
				...
				if(_this.treeData.length == 0){
					this.$http.post("/api/sysgroup/sysGroupTree").then(function(res){
