<template>
	<center-dialog ref="centerDialog" dialogTitle="选择部门" @centerCanclePreson="centerCanclePreson" dialogWidth="600px" dialogHeight="450px">
		<div slot="dialogContent">
			<div class="overflowHidden">
				<div class="mechanism fl">
					<div>
						<el-input
						  placeholder="请输入选择的部门"
						  v-model="filterText">
						</el-input>
	  ...
	  name: 'personDialog',
		...
		methods:{
			...
			selectedPresons(data,node,elem){
				let getCheckedNodesArr = this.$refs.personsTrees.getCheckedNodes();
				...
				getCheckedNodesArr.forEach((item,index)=>{
					...
					if(!item.isDept){
						...
						selectedPersonsIdArr.push(item.groupId);
			...
			showPersonDialog(chargeAppraiser,id,name){
				...
				if(_this.treeData.length == 0){
					this.filterText = '';   /*/sysgroup/getGroupAndUserTree*/
					this.$http.post("/api/sysgroup/sysOrgDepTreeByRole").then(function(res){
            _this.treeData = res.data;
            _this.$refs.personsTrees.setCheckedKeys(_this.selectPersonId);
			...
			clear(){
				this.selectPersonId = [];
				this.selectPersonName = [];
				this.$refs.personsTrees.setCheckedKeys([]);
			}
		},
		watch: {
	    	filterText(val) {
	        	this.$refs.personsTrees.filter(val);
		    }
	   	},
	}
</script>
