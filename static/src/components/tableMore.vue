    ...
    <div class="table80-wrap" :class="{checkedBG:(customDisplay && customDisplay.checkedBG)}">
        ...
        <div class="wrap-table" ref="tableWrap">
                ...
                <table>
                  <thead>
                      <tr v-for="(tableH,tableHInd) in tableHeader" :key="tableHInd">
                        <th v-for="(tableHVal,key) in tableH" :key="key" v-if="!tableHVal.isNoShow&&!tableHVal.isSpecial" v-show="!(cols10 && key==='operation')" :align="tableHVal.align?tableHVal.align:'center'" :rowspan="tableHVal.rowspan?tableHVal.rowspan:1" :colspan="tableHVal.colspan?tableHVal.colspan:1">
                          <div  @contextmenu.prevent="contextmenuShow($event,'del')" v-if="key==='check'" class="cell" :style="{'width':tableHVal.width?tableHVal.width+'px':'','min-width':tableHVal.minWidth?tableHVal.minWidth+'px':''}"><el-checkbox @change="checkAllFun" v-model="checkAll" :disabled="!$store.state.readonly">全选</el-checkbox></div>
                          <div v-else class="cell" :class="{searchM:tableHVal.search}" :style="{'width':tableHVal.width?tableHVal.width+'px':'','min-width':tableHVal.minWidth?tableHVal.minWidth+'px':''}">
                ...
                <table class="tableBody clusterize-content" ref="tableBody" v-if="sheetName!='利润表'&&sheetName!='资产负债表'&&sheetName!='审定数'" :style="{height: contentHeight}">
                    <tbody id="tableExampleContent">
                        <tr v-for="(tableR,tableRInd) in tableRowData" @click="addBg(tableRInd)" :key="tableRInd">
                            <td v-for="(tableHVal,key,indx) in tableHeader[tableHeader.length-1]" :key="key" v-if="!tableR.trIsHide" v-show="!(cols10 && key==='operation')" :align="tableHVal.align?tableHVal.align:'center'"  :class="tableHVal.align?tableHVal.align:'center'">
                                <!-- 操作 -->
                                ...
                                <div v-else-if="key==='check'" class="cell" :style="{'width':tableHVal.width?tableHVal.width+'px':'','min-width':tableHVal.minWidth?tableHVal.minWidth+'px':''}">
                                  <el-checkbox v-model="tableR[key]" @change="checkedFn(tableR[key],tableR)" :disabled="!$store.state.readonly"></el-checkbox>
                                </div>
                                <!-- 序号 -->
                                ...
                                @contextmenu.stop.prevent="sign($event,tableR,key,tableRInd,'mid')" @click="setCheckedBG($event,tableR,key,tableRInd)" :class="{checkedBlueCell:checkedBgObj.id==tableR.id&&checkedBgObj.key==key&&tableR[key]!=''}">
                                    <el-input :class="{isWarning:tableR[key]=='#N/A',isPasted:(tableR.paste&&pasteCloum.indexOf(key)>-1)}"  v-if="(tableR['disableColumns'].split(';')).indexOf(key)>-1" :disabled="true" v-model="tableR[key]"></el-input>
                                    <el-select v-else-if="tableHVal.selected" v-model="tableR[key]" :disabled="tableR.id==''||!$store.state.readonly  || cols10 || (customDisplay && customDisplay.disabled)" @change="blurInp(tableR,key,$event,tableRInd,'作价规则')"
                                    ...
                                    <el-select v-else-if="tableHVal.isSelect" v-model="tableR[key]"  @change="blurInp(tableR,key,$event,tableRInd,'是否进口')" :disabled="!$store.state.readonly || cols10 || (customDisplay && customDisplay.disabled)"
                                    :class="{isMarked:(tableR['columnNames'].split(',')).indexOf(key)>-1,isPasted:(tableR.paste&&pasteCloum.indexOf(key)>-1)}"
                                     filterable
                                    ...
                                    <el-date-picker v-else-if="tableHVal.search && tableHVal.search.inputType=='year'" v-model="value5" type="year" placeholder="选择年" :disabled="!$store.state.readonly || cols10 || (customDisplay && customDisplay.disabled)"></el-date-picker>
                                    <el-input v-else  v-model="tableR[key]" @keyup.native="keyupInp" @blur="blurInp(tableR,key,$event,tableRInd,'',tableHVal)" @focus="focusInp(tableR,key,$event)"
                                    	:class="{isWarning:tableR[key]=='#N/A',isMarked:(tableR['columnNames'].split(',')).indexOf(key)>-1,isPasted:(tableR.paste&&pasteCloum.indexOf(key)>-1)}"
                                      :disabled="!$store.state.readonly || cols10 || (customDisplay && customDisplay.disabled)"
                                      @paste.native.capture.prevent="pasteFromExcel($event,tableR,key,tableRInd,tableHVal,indx)">
                                    </el-input>
                                </div>
            ...
            <div class="wrap-table-fixed" :class="{transparent:resultData.length===0}"  ref="tableFixed" v-show="sheetName!='利润表'&&sheetName!='资产负债表'&&sheetName!='审定数'">
              <div class="wrap-table-head" @mousemove.prevent='mousemove($event)'>
                ...
                <table>
                   <thead>
                       <tr v-for="(tableH,tableHInd) in tableHeader" :key="tableHInd">
                          <th v-for="(tableHVal,key) in tableH" :key="key" v-if="!tableHVal.isNoShow&&!tableHVal.isSpecial&&tableHVal.fixed" v-show="!(cols10 && key==='operation')" :align="tableHVal.align?tableHVal.align:'center'" :rowspan="tableHVal.rowspan?tableHVal.rowspan:1" :colspan="tableHVal.colspan?tableHVal.colspan:1">
                            <div  @contextmenu.prevent="contextmenuShow($event,'del')" v-if="key==='check'" class="cell" :style="{'width':tableHVal.width?tableHVal.width+'px':'','min-width':tableHVal.minWidth?tableHVal.minWidth+'px':''}">
                              <el-checkbox @change="checkAllFun" v-model="checkAll" :disabled="!$store.state.readonly">全选</el-checkbox>
                            </div>
                            <div v-else class="cell" :class="{searchM:tableHVal.search}" :style="{'width':tableHVal.width?tableHVal.width+'px':'','min-width':tableHVal.minWidth?tableHVal.minWidth+'px':''}">
                                <span  class="searchMSpan"  @click.stop="popup(tableHVal,key,$event,tableHVal.label,'fixed')">
                                    {{tableHVal.label}}
                                </span>
                                  <div v-if="tableHVal.isFilter" class="tableHeadFilter" >
                                    <i class="triangle-up" title="升序" @click.stop="tableSort('asc',tableHVal,key,$event,tableHVal.search)" ref="tableSortAsc"></i>
              ...
              <div class="wrap-scroll" >
                <div class="wrap-table-body" @mouseover="mouseoverFun('left')" @scroll="loadMore($event, 'left')" @contextmenu.prevent="contextmenuShow" ref="tableFixedBody" id="tableFixedBody">
                  <table class="fixed-wid-table" :style="{height: contentHeight}" id="fixed_wid_table">
                    ...
                    <tr v-for="(tableR,tableRInd) in tableRowData" @click="addBg(tableRInd)" :key="tableRInd">
                      <td v-for="(tableHVal,key,indx) in tableHeader[tableHeader.length-1]" :key="key" v-if="!tableR.trIsHide&&tableHVal.fixed" v-show="!(cols10 && key==='operation')" :align="tableHVal.align?tableHVal.align:'center'" :class="tableHVal.align?tableHVal.align:'center'">
                        <!-- 操作 -->
                        <div v-if="key==='operation'" class="cell" :style="{'width':tableHVal.width?tableHVal.width+'px':'','min-width':tableHVal.minWidth?tableHVal.minWidth+'px':''}">
                          <span  :class="{iconGroup:sheetName=='房屋建筑物'}">
                            <img class="operation-icon operationHover" src="/static/img/operation.png" alt="" @click.stop="operationFun($event,tableR,tableHVal)">
                            <!--需要直接点击右边栏-->
                            <template v-if="tableR.xlsCaseInfoList&&externalFlag">
                                <template  v-for="caseItem in tableR.xlsCaseInfoList">
                                  ...
                                  <template v-if="caseItem.ruleType==2">
                                      <i class="case-icon" :class="{actived:caseItem.enableSheetName!=''}" @click="showCasePanel(tableR,caseItem.ruleType)">收</i>
                                  </template>
                                  <template v-if="caseItem.ruleType==3">
                                      <i class="case-icon" :class="{actived:caseItem.enableSheetName!=''}" @click="showCasePanel(tableR,caseItem.ruleType)">市</i>
                                  </template>
                                </template>
                            </template>
                          </span>
                        ...
                        <div v-else class="cell"
                          ...
                          <el-select v-if="tableHVal.selected" v-model="tableR[key]" :disabled="tableR.id==''||!$store.state.readonly  || cols10 || (customDisplay && customDisplay.disabled)" @change="blurInp(tableR,key,$event,tableRInd,'作价规则')"
                                     :class="{isMarked:(tableR['columnNames'].split(',')).indexOf(key)>-1,isPasted:(tableR.paste&&pasteCloum.indexOf(key)>-1)}" :ref="'select'+tableR.numB+key" @focus="selectMenuDown(tableR,key)">
                            <el-option
                              v-for="item in tableHVal.search.rule"
                              :key="item.value"
                              :label="item.label"
            ...
            <div class="table-footer">
              <div  ref="tableFoot" @scroll="scrollFoot" class="tableTotal"   :class="{balanceTableFd:sheetName=='资产负债表',noTopBorder:sheetName=='利润表'||sheetName=='审定数'}">
                <table>
                    <tbody>
                        <tr v-for="(tableT,tableTInd) in totalData" :key="tableTInd">
                            <td v-for="(tableH,key) in tableHeader[tableHeader.length-1]" v-show="!(cols10 && key==='operation')" :key="key" :align="tableH.align?tableH.align:'center'" :class="tableH.align?tableH.align:'center'">
                                <div class="cell" :style="{'width':tableH.width?tableH.width+'px':'','min-width':tableH.minWidth?tableH.minWidth+'px':''}">
                                    <template v-if="tableH.footLabel">
                                        <span v-if="tableH.footLabel[tableTInd+'']=='合计'">
                                    ...
                                    <template v-else>
                                      ...
                                      <span v-else class="pr5">{{tableT[key]}}</span>
                                  </template>
                                </div>
                            </td>
                        </tr>
              ...
              <div class="tableTotal fixed-table-taotal">
                <table>
                  <tbody>
                    <tr v-for="(tableT,tableTInd) in totalData" :key="tableTInd">
                      <td v-for="(tableH,key) in tableHeader[tableHeader.length-1]" v-show="!(cols10 && key==='operation')" :key="key" :align="tableH.align?tableH.align:'center'" :class="tableH.align?tableH.align:'center'" v-if="tableH.fixed">
                        <div class="cell" :style="{'width':tableH.width?tableH.width+'px':'','min-width':tableH.minWidth?tableH.minWidth+'px':''}">
                            <template v-if="tableH.footLabel">
                                <span v-if="tableH.footLabel[tableTInd+'']=='合计'">
                                  {{tableH.footLabel[tableTInd]}}(共{{totalNum}}条已入库)
        ...
        <div class="contextmenu" ref="contextmenu" :class="{hidden:isHideMenu}">
            <ul>
                ...
                <li v-if="sheetName=='房屋建筑物'&&externalFlag&&caseEntranceFlag" @click="showCase">查看评估案列</li>
            </ul>
        </div>
        <!-- 操做弹窗 -->
        <div class="contextmenu operation" ref="operation" :class="{hidden:isHideOperation}">
            <ul>
                ...
                <li v-if="isShowProspecting" @click="selectPeople" :class='{"isdisabled":this.tableInfo.id == ""}'><i class="iconfont icon-daiban"></i>勘察</li>
...
export default {
  ...
  mounted() {
    ...
    // Public.$on('addCaseIconFn',(type,enable)=>{
    //   this.addCaseIconFn(type,enable)
    // })
  ...
  methods: {
    ...
    showCase(){
      ...
      var idx = Number(checkArr[0].numB)-1;
      this.$store.state.caseDataId = checkArr[0].id;
      this.$store.state.curCaseInfo = checkArr[0];
    ...
    */
    addCaseIconFn(type,enableSheetName,status){
      ...
      if(this.$store.state.curCaseInfo){
        index = this.resultData.indexOf(_this.$store.state.curCaseInfo);
        arr = this.$store.state.curCaseInfo.xlsCaseInfoList||[];

      }else{
        checkObj = this.getChecked();
        index = this.resultData.indexOf(checkObj[0]);
        arr = checkObj[0].xlsCaseInfoList||[];
      }
      if(arr.length>0){
              ...
              case "启用":
                 ...
                 for(var j =0;j<this.resultData.length;j++){
                    for(var t =0;t<this.resultData[j].xlsCaseInfoList.length;t++){
                        if(this.resultData[j].xlsCaseInfoList[t].ruleType==type){
    ...
    openHouseCase(){
      ...
      this.$store.state.componentsType = "houseCase"; //现场勘查
    ...
    contextmenuShow(e, del) {
     ...
     if(this.sheetName=='利润表'||this.sheetName=='资产负债表'||this.sheetName=='审定数') return
      ...
      //需要判断是否显示“查看案列”(当前只有一个勾选)

      if(this.sheetName=="房屋建筑物"&&checkArr.length==1&&checkArr[0].id&&(checkArr[0].ruleType==2||checkArr[0].ruleType==6)){
          this.caseEntranceFlag = false;
      }
      ...
      if(more){
        if (this.resultData.length==0) {
          ...
          for (let i = index+1; i < _this.cellNums+index+1; i++) {
            var addRowObj = Object.assign({},_this.initData);
            addRowObj.sort = this.resultData[index].sort;
        ...
        // }
        addRowObj.sort = this.resultData[index].sort+1;
    ...
    searchCallback(data) {
      ...
      console.log(this.searchCondition, 22222);
      ...
      if (this.inputCont!= tableR[key]) {
        ...
        if (tableR.id) {
          ...
          if (_this.tableRows[idx - 1]) {
            sort = _this.tableRows[idx - 1].sort;
            if(!sort) sort=8;
          } else {
            sort = 8;
          }
  ...
      }
    ...
    */
    ...
    getSheetData(callback, type,reload,top) {
      ...
      if(this.sheetName=='房屋建筑物'&&this.$store.state.caseTxt=="显示全部"){
           onlyCase = true;
      }else{
          onlyCase = false;
      }
...
          })
      ...
      this.$parent.getUploadStatu(function(){
        ...
        .then(function(data) {
         ...
         if(_this.updataLoading) _this.updataLoading.close();
        })
        .catch(function(error) {
          _this.loadDataFlag = false;
          _this.$store.state.isSavingFlag = false;
    ...
    reckonHg(){
      ...
      this.$refs.tableScroll.style.height = oH-120-distance-headSearchH+'px';
      if(this.$refs.tableFixedBody){
         this.$refs.tableFixedBody.style.maxHeight = oH-120-distance-headSearchH+'px';
      }
      ...
      if(this.sheetName=="资产负债表"){
        this.$refs.tableScroll.style.height = parseInt(this.$refs.tableScroll.style.height)-headSearchH+18+'px'
      }
       for(var i =0 ;i<oDivs.length;i++){
        if(oDivs[i].style.display!="none"){
          ...
          oDivs[i].querySelector('.wrap-table-fixed').querySelector('.wrap-table-body').style.height= oDivs[i].querySelector('.wrap-table-body').style.height = parseInt(_this.$refs.tableScroll.style.height)-searchContH+'px';
        }
      }
     _this.renewVisibleData(_this.divScrollTop);
    },
...
.iconGroup{display: inline-block;text-align: left;width: 78px;
