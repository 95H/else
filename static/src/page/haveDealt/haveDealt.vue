<template>
	<div class="to-dealt-content">
		...
		<div class="white-bg">
			<dealt-list :messageToDeal="messageToDeal" :totalDataNum="totalDataNum" :type="type">
				<span slot="btn" class="btn-hook search-btn" @click="allRead">全部清空</span>
			</dealt-list>
	...
	export default {
		...
		methods: {
			...
			allRead() {
				let ids = [],
					_this = this;
				_this.messageToDeal.forEach(item => {
					ids.push(item.messagetoreadId)
				})
				let data = {
					ids: ids.join()
				}
				_this.$confirm('您是否确认删除?', '提示', {
					...
					_this.$http.post('api/message/setIsRead', data)
						.then(res => {
							_this.searchList();
							_this.$messageSuccess('清空成功！')
						})
						.catch(err => {
							_this.$messageError(err)
						})
				}).catch(() => {
					_this.$messageSuccess('已取消操作')
				})
