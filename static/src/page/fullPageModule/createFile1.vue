<template>
    ...
    <div class="zlpg-assessmentDescription-right">
      ...
      <div class="editorBox" v-show='!loading'>
      	<div id="editorToolbar"></div>
      	<div id="editorContent" @click="handleMenu = false" @keyup="handleMenu = false" @contextmenu="insertRelation"></div>
      </div>
    </div>
    <!-- 添加目录 -->
    ...
    <ul class="handleMenuFun" v-show="handleMenu" ref="contextmenu">
      <li v-if="isRelation && lockFalg=='yes' && sign != 'MARKET' && sign != 'PROFIT' && sign != 'AL'" @click="customRelation">自定义关联</li>
      ...
      <li v-if="!addRelation && sign != 'MARKET' && sign != 'PROFIT' && sign != 'AL'" @click="openRelationDetail">查看明细表</li>
    </ul>
    <!-- 删除关联确定弹窗 -->
...
export default {
  ...
  methods: {
    ...
    changeRelationFun(oldId,newId,tabId,type,val){
      $("#editorContent span_"+oldId).after('<span_'+newId+'><danger contenteditable="false" tabId="'+ tabId +'" id="'+ newId +'" type="'+ type +'">'+val+'</danger></span_'+newId+'>')
      $("#editorContent span_"+oldId).remove();
    ...
    delRelationDialog(e){
      ...
      this.delRelationText = e.target.innerText;
      this.handleMenu = false;
    },
    // 删除关联
    delRelationFun(){
    ...
    updataEditorContent(){
      this.diffDataArr = this.createEditorArr(this.editor.txt.getJSON());
      this.editorContent = this.editor.txt.html();
      this.editor.txt.html(this.editorContent);
      this.dangerLength = $("#editorContent danger").length;
    ...
    //对比重要文件的内容
    setDiffData: function(html) {
      // var _this = this,
      //   num = 0;
      // this.diffData = this.editor.txt.getJSON();
      // diffDataX(this.diffData);
      // if (num != this.diffDataArr.length) {
      //   _this.$messageError("当前数据已经锁定，不能修改");
      //   _this.editor.txt.html(this.editorCont);
      //   return;
      // } else {
      //   this.editorCont = html;
      // }
      // function diffDataX(diffData) {
      //   for (var i = 0; i < diffData.length; i++) {
      //     if (diffData[i] && diffData[i].tag) {
      //       if (diffData[i].tag == "danger") {
      //         if (diffData[i].children[0] != _this.diffDataArr[num]) {
      //           _this.$messageError("当前数据已经锁定，不能修改");
      //           _this.editor.txt.html(_this.editorCont);
      //           return;
      //         }
      //         num++;
      //       } else {
      //         diffDataX(diffData[i].children);
      //       }
      //     }
      //   }
      // }

      if ($("#editorContent danger").length != this.dangerLength) {
    ...
    postFunction(node) {
      ...
      _this.$http
        ...
        .then(function(res) {
          ...
          _this.dangerLength = $("#editorContent danger").length;
          // 删除多余数据
          ...
          if (_this.lockFalg == "yes" && _this.readonly) {
            _this.editor.$textElem.attr("contenteditable", true);
          } else{
            _this.editor.$textElem.attr("contenteditable", false);
          }
          //判断是不是收益法
          if (/MARKET/.test(res.data.sign)) {
            _this.sign = 'MARKET';
          }else if(/PROFIT/.test(res.data.sign)){
          	_this.sign = 'PROFIT';
          }else if(/AL/.test(res.data.sign)){
          	_this.sign = 'AL';
          }else {
          ...
          _this.diffDataArr = _this.createEditorArr(
            _this.editor.txt.getJSON()
          );
          _this.editorCont = _this.editor.txt.html();
        })
    ...
    saveContent(isSfFlag, callback,investorId,paramsUrl) {
      ...
      if (isSfFlag) {
        _this.saveNodeName(
          {
            directoryContent: _this.editor.txt.html(),
            id: _this.thisNodeId,
            projectId: _this.$store.state.projectId,
            investorId,
            isSf: "1",
    ...
    flush() {
      ...
      _this.$http
        ...
        .then(function(res) {
          ...
          // 删除多余数据
          _this.delMoreData(res.data);
          _this.diffDataArr = _this.createEditorArr(_this.editor.txt.getJSON());
          _this.editorCont = _this.editor.txt.html();
          _this.lockFalg = res.data.lockFalg;
          _this.loading=false;
          _this.loadTxt=" ";
    ...
    delMoreData(data){
      ...
      if (_this.$route.params.tabId == '7') {
        if(data.sign == 'REPORT_DIRECTORY_CONTENT_7'){
          $("span_20104c9b13aab92866ed5c78a376ffde").remove();
        }
        if(data.sign == 'REPORT_DIRECTORY_CONTENT_3'){
          $("span_ef299e201b7c6546255be2ca8b16b9ea").remove();
        }
      }
      if (_this.$route.params.tabId == '8') {
