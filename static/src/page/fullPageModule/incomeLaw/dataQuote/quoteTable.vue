    ...
    <div class="wrap-table" ref="tableWrap">
          ...
          <thead>
            <tr v-for="(tableH,tableHInd) in tableHeader" :key="tableHInd">
              <th
                ...
                :key="key"
                v-if="!tableHVal.isNoShow&&!tableHVal.isSpecial&&key!='operation'"
          ...
          <tbody id="tableExampleContent">
            <tr v-for="(tableR,tableRInd) in tableRowData" :key="tableRInd">
              <td
                ...
                :key="key"
                v-if="!tableR.trIsHide&&key!='operation'"
                :align="tableHVal.align?tableHVal.align:'center'"
              ...
              >
                ...
                >
                  <el-input
                    ...
                    v-if="tableR['disableColumns']&&(tableR['disableColumns'].split(';')).indexOf(key)>-1"
                    :disabled="true"
                    v-model="tableR[key]"
                  ></el-input>
            ...
            <tr v-for="(tableR,tableRInd) in tableRowData" :key="tableRInd">
              ...
              >
                <div
                  ...
                  :style="{'width':tableHVal.width?tableHVal.width+'px':'','min-width':tableHVal.minWidth?tableHVal.minWidth+'px':''}"
                >
                  <el-input v-if="tableHVal.isNoEdit" :readonly="true" v-model="tableR[key]"></el-input>
            ...
            <thead>
              <tr v-for="(tableH,tableHInd) in tableHeader" :key="tableHInd">
                <th
                  v-for="(tableHVal,key) in tableH"
                  :key="key"
                  v-if="!tableHVal.isNoShow&&!tableHVal.isSpecial&&tableHVal.fixed&&key!='operation'"
                  v-show="!(cols10)"
                  :align="tableHVal.align?tableHVal.align:'center'"
                  :rowspan="tableHVal.rowspan?tableHVal.rowspan:1"
                  :colspan="tableHVal.colspan?tableHVal.colspan:1"
              ...
              <tbody id="fix_wid_tbody">
                <tr v-for="(tableR,tableRInd) in tableRowData" :key="tableRInd">
                  <td
                    v-for="(tableHVal,key,indx) in tableHeader[tableHeader.length-1]"
                    :key="key"
                    v-if="!tableR.trIsHide&&tableHVal.fixed&&key!='operation'"
                    :align="tableHVal.align?tableHVal.align:'center'"
                    :class="tableHVal.align?tableHVal.align:'center'"
                  >
                    <!-- 单选 -->
      ...
      <div class="table-footer">
        <div
          ...
          @scroll="scrollFoot"
          class="tableTotal"
          :style="{borderTop:$store.state.qutoTab==='数据列表'?'none':''}"
          :class="{balanceTableFd:sheetName=='资产负债表',noTopBorder:sheetName=='利润表'||sheetName=='审定数'}"
        >
          <table>
            <tbody :style="{visibility:$store.state.qutoTab==='数据列表'?'hidden':''}">
        ...
        <!--固定的列 begin-->
        <div class="tableTotal fixed-table-taotal" :style="{visibility:$store.state.qutoTab==='数据列表'?'hidden':'',borderTop:$store.state.qutoTab==='数据列表'?'none':''}">
          <table>
            <tbody>
              <tr v-for="(tableT,tableTInd) in totalData" :key="tableTInd">
                <td
                  ...
                  :align="tableH.align?tableH.align:'center'"
                  :class="tableH.align?tableH.align:'center'"
                  v-if="tableH.fixed&&key!='operation'"
                >
                  <div
                    class="cell"
                    :style="{'width':tableH.width?tableH.width+'px':'','min-width':tableH.minWidth?tableH.minWidth+'px':''}"
...
<script>
import incomeSearch from "./incomeSearch.vue";
//绘制
...
export default {
  ...
  mounted() {
    ...
    if (this.sheetNameBool && this.firstLoad) {
  ...
  methods: {
    ...
    removeQuote() {
      ...
      this.confirmCase("确认移出以上数据？", function() {
        _this.confirmQuoteCallBack(ids, checkArr, "移除");
      });
    },
    /*提示 */
    ...
    confirmQuote() {
      ...
      this.confirmCase("确认引用以上数据？", function() {
        _this.confirmQuoteCallBack(ids, checkArr);
      });
    },
    confirmQuoteCallBack(ids, checkArr, type) {
      var _this = this;
      if (type == "移除") {
      ...
      this.$nextTick(() => {
        if (
          this.resultData[this.resultData.length - 1] &&
    ...
    getSheetData(callback, type, reload, top) {
      ...
      var url;
      //数据列表
      if (this.$store.state.qutoTab == "数据列表") {
        url = "/api/overflowLongThrow/getList";
      } else {
        url = "/api/overflowLongThrow/getMoveData";
      }
      ...
      this.$http
        ...
        .then(function(data) {
          ...
          _this.pageMax += pageData.data.length;
            ...
            for (var item in tableHeader) {
              ...
              if (tableHeader[item].isThousands) {
                if (
                  !(
                    pageData.data[i][item] === "" ||
                    pageData.data[i][item] === "0"
                  )
                ) {
                  pageData.data[i][item] = _this.$store.state.commafyNumber(
                    pageData.data[i][item]
                  );
                }
              }
          ...
          if (_this.firstShowLoading) {
            ...
            // }else{
            //   _this.totalData.push(_this.initData);
            // }
            _this.totalData.push({});
            if(_this.$store.state.qutoTab!='数据列表'){
              _this.getTotalNum();
            }
            _this.resultData = pageData.data;
          } else {
            switch (_this.direction) {
              case "dn":
                ...
                for (var i = 0; i < pageData.data.length; i++) {
                  ...
                  pageData.data[i].numB = order;
                }
                _this.resultData = _this.resultData.concat(pageData.data);
                //进行绘制的时候会有延时
    ...
    deletData(id, operation) {
      ...
      var params = {};
      var url = "";
      this.$store.state.olreadyOperation = true;
      if (id == "") return;
      if (id != "all") {
        var arr = id.split(",");
      ...
      if (operation == "移除") {
        url = "/api/overflowLongThrow/deleteSelectByIdKeys";
        params = {
          idKey: id,
          projectId: _this.sendParams.projectId,
          investorId: _this.sendParams.investorId,
          conditions: _this.sheetSearchTerm.conditions,
          sheetName: _this.sheetName,
          sheetNameId: this.$store.state.quoteObj.sheetNameId || ""
        };
      } else {
        url = "/api/overflowLongThrow/confirmCitation";
        params = {
          idStr: id,
          projectId: _this.sendParams.projectId,
          investorId: _this.sendParams.investorId,
    ...
    updataTable(id, property, value, type, idx, isTotal, ratioPrice, data) {
      ...
      if (type && id == "") {
        this.$parent.getLoadingText("批量修改中");
      }
      ...
      _this.$parent.getUploadStatu(function() {
        _this.$http
          ...
          .then(function(data) {
            ...
            _this.resultData.splice(
              _this.tableRows[idx].numB - 1,
              1,
              data.data
            );
            _this.loadDataFlag = false;
    ...
    getTotalNum() {
      ...
      var arr = [];
      this.$http
        .post("/api/overflowLongThrow/getTotal", {
          sheetName: _this.sheetName,
          projectId: _this.sendParams.projectId,
          investorId: _this.sendParams.investorId,
          showLoading: false
    ...
    reckonHg() {
      ...
      var oH = parseInt(this.$store.state.cprRightHg - 78);
